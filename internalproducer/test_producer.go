...
func (tp *TestProducer) generateWorkerData(ctx context.Context, workerID, recordCount, batchSize int) (int, error) {
	...
	for i := 0; i < recordCount; i++ {
		...
		serializer, err := avro.NewSerializer("schema/seed_status.avsc")
		if err != nil {
			tp.logger.Error("Failed to create Avro serializer", zap.Error(err))
			continue
		}

		avroData, err := serializer.Serialize(record)
		if err != nil {
			tp.logger.Error("Failed to serialize to Avro", zap.Error(err))
			continue
		}
